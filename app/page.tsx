...
const games: Game[] = [
  { title: "컴파일: 메인 1", publisher: "만두게임즈", type: "2인", weight: "적당히", players: "2인", time: "20–30분", status: "대표작", description: "AI 테마 덱으로 세 전선을 두고 겨루는 카드 전략전. 높은 카드는 인장, 낮은 카드는 능력으로 쓰는 선택이 핵심입니다.", audience: "짧고 치열한 2인전을 찾는 사람", video: "컴파일 메인 1 보드게임", videoId: "Gd2H-EcTzTM" },
  ...
  { title: "두냐자드의 모험", publisher: "코리아보드게임즈", type: "협력", weight: "가볍게", players: "2–5인", time: "15분", status: "콘 신작", description: "90초 안에 이야기와 맞는 타일을 함께 찾아내는 실시간 협력 어드벤처입니다.", audience: "가족·입문자 협력 플레이", video: "두냐자드의 모험 보드게임" },
  { title: "새우깡 잡기", publisher: "코리아보드게임즈", type: "가족", weight: "가볍게", players: "2–4인", time: "10분", status: "콘 신작", description: "같은 새우깡 카드를 짝지어 버리고 갈매기 카드를 피하는 빠른 카드게임입니다.", audience: "어린이와 가볍게 즐길 가족", video: "새우깡 잡기 보드게임" },
  { title: "블러핏", publisher: "코리아보드게임즈", type: "파티", weight: "가볍게", players: "3–6인", time: "15분", status: "콘 신작", description: "숨긴 카드와 상대의 선택을 읽어 중앙 카드를 가져오는 블러핑 게임입니다.", audience: "상대 표정을 읽는 파티 게임 팬", video: "블러핏 보드게임" },
  { title: "데스 스테이션", publisher: "코리아보드게임즈", type: "추리", weight: "적당히", players: "2–4인", time: "30분", status: "콘 신작", description: "타일을 놓고 단서를 좁혀 가며 사건을 풀어내는 추리 퍼즐입니다.", audience: "추리와 공간 퍼즐을 함께 좋아하는 사람", video: "데스 스테이션 보드게임" },
  { title: "세티: 우주 기관", publisher: "보드피아", type: "전략", weight: "깊게", players: "1–4인", time: "90–150분", status: "콘 신작", description: "비대칭 우주기관 11개와 외계종족 3종, 새 프로젝트를 더하는 《세티》 확장입니다.", audience: "세티를 깊게 즐긴 전략 게이머", video: "세티 우주 기관 보드게임", image: "/covers/seti.png" },
  { title: "엠버하트", publisher: "보드피아", type: "전략", weight: "적당히", players: "1–4인", time: "45–75분", status: "콘 신작", description: "일꾼을 배치해 드래곤을 구조·훈련하고 밀렵꾼과 맞서는 판타지 전략게임입니다.", audience: "테마 있는 일꾼 놓기 입문자", video: "엠버하트 보드게임" },
  { title: "죄악의 카니발", publisher: "보드피아", type: "파티", weight: "가볍게", players: "3–6인", time: "20분", status: "콘 신작", description: "일곱 죄악 카드를 비밀리에 고르고 주사위를 가져오며 상대를 방해하는 블러핑 게임입니다.", audience: "가벼운 견제와 심리전을 원하는 모임", video: "죄악의 카니발 보드게임" },
  { title: "태그 팀: 아서왕 전설", publisher: "아스모디", type: "2인", weight: "적당히", players: "2인", time: "20–30분", status: "콘 신작", description: "미리 구성한 카드 순서가 자동 전투를 벌이는 2인 덱빌딩 오토배틀러. 독립 플레이가 가능합니다.", audience: "전투 덱빌딩을 간결하게 즐기고 싶은 2인", video: "태그 팀 아서왕 전설 보드게임" },
  { title: "플라워즈", publisher: "보드엠", type: "가족", weight: "가볍게", players: "2–4인", time: "20분", status: "콘 신작", description: "숫자 카드의 등장 빈도와 배치 조건으로 꽃밭을 완성하는 카드 퍼즐입니다.", audience: "예쁜 테이블 게임을 원하는 가족", video: "플라워즈 보드게임", image: "/covers/flowers.jpg" },
  { title: "P.I.", publisher: "보드엠", type: "추리", weight: "적당히", players: "2–5인", time: "45분", status: "콘 신작", description: "경쟁자보다 먼저 자신의 사건을 해결해야 하는 경쟁형 추리게임입니다.", audience: "단서 조합과 추론을 좋아하는 사람", video: "P.I. 보드게임" },
  { title: "테이크 타임", publisher: "보드엠", type: "협력", weight: "적당히", players: "2–4인", time: "20분", status: "콘 신작", description: "제한된 의사소통만으로 숫자 카드를 시계 둘레에 배치하는 협력 퍼즐입니다.", audience: "말 없이 호흡을 맞추는 협력을 좋아하는 팀", video: "테이크 타임 보드게임", image: "/covers/taketime.png" },
  { title: "북 타워", publisher: "보드엠", type: "협력", weight: "가볍게", players: "1–4인", time: "15분", status: "콘 신작", description: "고양이와 책을 떨어뜨리지 않도록 함께 쌓는 협력 덱스터리티입니다.", audience: "아이와 함께할 손기술 게임", video: "북 타워 보드게임" },
...
export default function Home() {
  ...
  const videoEmbed = selectedGame.videoId ? `https://www.youtube-nocookie.com/embed/${selectedGame.videoId}` : `https://www.youtube.com/embed?listType=search&list=${encodeURIComponent(selectedGame.video)}`;

  return <main>
    <header className="nav"><a className="brand" href="#top">BGC<span>26</span></a><nav><a href="#map">MAP</a><a href="#vendors">VENDORS</a><a href="#watch">WATCH</a></nav><a className="nav-link" href="https://www.boardgamecon.com/" target="_blank" rel="noreferrer">OFFICIAL ↗</a></header>
    ...
    <section className="vendors-section" id="vendors"><div className="vendor-intro"><p className="mono">02 / EXHIBITORS & GAMES</p><h2>부스마다<br/>다른 한 수.</h2><p>탭을 고르면 해당 업체의 공개 신작·대표작과 상세 플레이 정보를 보여줍니다.</p></div><div className="vendor-content"><div className="vendor-tabs" role="tablist" aria-label="참가 업체">{vendorTabs.map((vendor) => <button key={vendor} role="tab" aria-selected={selectedVendor === vendor} className={selectedVendor === vendor ? "active" : ""} onClick={() => selectVendor(vendor)}>{vendor}</button>)}</div><div className="game-tools"><div className="filter-row">{["전체", "파티", "가족", "협력", "추리", "2인", "전략"].map((item) => <button className={filter === item ? "active" : ""} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="게임명 검색" aria-label="게임명 검색" /></label></div><p className="selection-label"><span className="mono">NOW VIEWING</span> {selectedVendor} <em>{filtered.length} GAMES</em></p><div className="game-cards">{filtered.map((game) => <article className={`game-card ${selectedGame.title === game.title ? "selected" : ""}`} key={game.title}>{game.image && <button className="game-cover" onClick={() => setSelectedGame(game)} aria-label={`${game.title} 상세 보기`}><img src={game.image} alt={`${game.title} 보드게임 박스`} /></button>}<button className="game-card-main" onClick={() => setSelectedGame(game)}><span className="mono">{game.status}</span><h3>{game.title}</h3><p>{game.description}</p><div><b>{game.type}</b><b>{game.weight}</b><span>{game.players} · {game.time}</span></div></button><a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(game.video)}`} target="_blank" rel="noreferrer">YouTube에서 찾기 ↗</a></article>)}</div></div></section>
    <section className="watch-section" id="watch"><div className="section-heading"><p className="mono">03 / WATCH PREVIEW</p><h2>게임을 고르면,<br/>영상으로 먼저.</h2></div><div className="watch-layout"><div className="video-frame"><iframe src={videoEmbed} title={`${selectedGame.title} YouTube 미리보기`} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div><aside className="video-detail"><span className="mono">{selectedGame.videoId ? "YOUTUBE RULES PREVIEW" : "YOUTUBE SEARCH PREVIEW"}</span><h3>{selectedGame.title}</h3><p>{selectedGame.description}</p><dl><div><dt>추천</dt><dd>{selectedGame.audience}</dd></div><div><dt>플레이</dt><dd>{selectedGame.players} · {selectedGame.time}</dd></div></dl><a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(selectedGame.video)}`} target="_blank" rel="noreferrer">YouTube에서 영상 전체 보기 ↗</a></aside></div></section>
