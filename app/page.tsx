...
export default function Home() {
  ...
  return <main>
    <header className="nav"><a className="brand" href="#top">BGC<span>26</span></a><nav><a href="#map">MAP</a><a href="#events">주요 이벤트</a><div className={`vendor-menu ${vendorMenuOpen ? "open" : ""}`}><button className="vendor-menu-trigger" type="button" aria-expanded={vendorMenuOpen} aria-controls="vendor-menu-panel" onClick={() => setVendorMenuOpen((open) => !open)}>참가 업체 <span>↓</span></button><div className="vendor-menu-panel" id="vendor-menu-panel"><button className="vendor-menu-all" onClick={() => selectVendor("전체")}>전체 업체<small>{vendorTabs.length - 1} VENDORS</small></button>{vendorTabs.filter((vendor) => vendor !== "전체").map((vendor) => <button key={vendor} onClick={() => selectVendor(vendor)}>{vendor}<small>{games.filter((game) => game.publisher === vendor).length} GAMES</small></button>)}</div></div></nav><a className="nav-link" href="https://www.boardgamecon.com/" target="_blank" rel="noreferrer">OFFICIAL ↗</a></header>
