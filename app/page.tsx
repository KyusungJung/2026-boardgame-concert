...
const gameId = (game: Pick<Game, "title" | "publisher">) => `${game.publisher}::${game.title}`;
...
const games: Game[] = [
  ...
  { title: "오목체스", publisher: "매직빈게임즈", type: "전략", weight: "적당히", players: "2인", time: "30분", status: "대표작", description: "군사와 기사를 움직여 먼저 오목 두 개를 완성하는 추상 전략 게임입니다. 영상 14:11 소개 장면을 반영했습니다.", audience: "체스와 오목 모두 좋아하는 2인", video: "오목체스 보드게임", image: "/video-stills/omok-chess-youtube.jpg", source: "https://youtu.be/om8G4KPQgso?si=ds66eY3VSwImR_gU&t=851", sourceLabel: "14:11 영상 소개 · 설명 출처 보기 ↗" },
...
type BoothPoster = { image: string; title: string; description: string; source: string };
const mandooEventSource = "https://www.instagram.com/p/Daui3XXE4IF/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA==";
const boothPosters: Record<string, BoothPoster[]> = {
  ...
  "코리아보드게임즈": [
    ...
    { image: "/booth-posters/image13.jpg", title: "인원별 체험 게임", description: "2·3·4인 이상 인원별 추천 체험 게임을 현장 상황에 따라 운영합니다.", source: kbgSource },
    { image: "/booth-posters/image34.jpg", title: "스플렌더 · 바다숲 대회", description: "7월 18일 13시 스플렌더 그랑프리 보드게임콘 예선, 7월 19일 14시 바다숲 토너먼트가 COEX 3층 컨퍼런스룸(남) 318호에서 열립니다.", source: kbgSource },
  ],
  "보드피아": [
    { image: "/booth-posters/image17.jpg", title: "세티 확장: 우주 기관", description: "공유 시트에는 세티 확장과 메탈 코인, 본판 재입고가 2026 보드게임콘 출시 항목으로 소개돼 있습니다.", source: "https://www.instagram.com/p/DaRsk7tExoy/" },
    { image: "/booth-posters/image24.jpg", title: "엠버하트 · 죄악의 카니발", description: "두 신작의 실물 패키지 이미지를 게임 목록 위에서 확인할 수 있습니다.", source: "https://www.instagram.com/p/DaRsk7tExoy/" },
  ],
  "데블다이스": [
    { image: "/booth-posters/image35.jpg", title: "구매 혜택 이벤트", description: "게임 구매 시 데블다이스 보드카페 이용권을 제공하고, 결제 금액별 캐주얼·전략 게임 혜택을 안내합니다.", source: "https://www.instagram.com/p/DakeJ0FE-pD/" },
    { image: "/booth-posters/image16.jpg", title: "추가 프로모션", description: "마녀들의 경주·페이퍼 던전 구매 프로모션과 5만 원 이상 무료 택배 서비스를 안내합니다.", source: "https://www.instagram.com/p/DakeJ0FE-pD/" },
    { image: "/booth-posters/image37.jpg", title: "TANTRIX 스피드 챌린지", description: "퍼즐을 완성하는 현장 스피드 챌린지입니다. 1등은 데드 바이 데이라이트 컬렉터스·확장 풀세트, 참여 선물도 안내돼 있습니다.", source: "https://www.instagram.com/p/DakeJ0FE-pD/" },
  ],
  "매직빈게임즈": [
    { image: "/booth-posters/image33.jpg", title: "오목체스 반값 할인 · 대회", description: "오목과 체스를 결합한 오목체스의 반값 할인과 대회 신청을 안내합니다.", source: "https://www.instagram.com/p/DZo-JK6R4QI/" },
    { image: "/booth-posters/image18.jpg", title: "첫 오목체스 챔피언십", description: "7월 18일 16:00–17:50, COEX 3층 남 308호에서 32강 1대1 방식으로 진행됩니다. 선착순 모집입니다.", source: "https://www.instagram.com/p/DZo-JK6R4QI/" },
  ],
  "MTS": [
    { image: "/booth-posters/image25.jpg", title: "체험 아케이드 뽑기", description: "MTS 게임을 체험해 코인을 모으고 뽑기를 즐기는 이벤트입니다. 게임당 코인 1개, 최대 4개까지 안내돼 있습니다.", source: "https://www.instagram.com/p/DawbJVkEhwo/" },
  ...
  "행복한 바오밥": [
    ...
    { image: "/booth-posters/image36.jpg", title: "대회 · 사인회 · 구매 혜택", description: "정고 토너먼트, 플레이 제주 일러스트레이터 사인회와 일부 게임 구매 증정·선주문 안내가 함께 게시돼 있습니다.", source: "https://www.instagram.com/p/DaSNZ5riZLd/" },
  ],
  "아스모디": [
...
export default function Home() {
  ...
  const [wishlistSort, setWishlistSort] = useState<"priority" | "vendor">("priority");
  ...
  const wishlistGroups = useMemo(() => {
    const sorted = [...wishlist].sort((a, b) => wishlistSort === "priority" ? a.priority - b.priority || a.addedAt - b.addedAt : a.publisher.localeCompare(b.publisher, "ko") || a.priority - b.priority);
    return sorted.reduce<Record<string, WishlistItem[]>>((groups, item) => { (groups[item.publisher] ??= []).push(item); return groups; }, {});
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) setWishlist(parsed.filter((item): item is WishlistItem => Boolean(item && typeof item === "object" && "id" in item && "title" in item && "publisher" in item && "priority" in item && "addedAt" in item)));
      }
    } catch { window.localStorage.removeItem(WISHLIST_STORAGE_KEY); }
    setWishlistLoaded(true);
  }, []);
  useEffect(() => { if (wishlistLoaded) window.localStorage.setItem(WISHLIST_STORAGE_KEY, JSON.stringify(wishlist)); }, [wishlist, wishlistLoaded]);
  useEffect(() => {
    if (!wishlistOpen) return;
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setWishlistOpen(false); };
    window.addEventListener("keydown", closeOnEscape);
    return () => window.removeEventListener("keydown", closeOnEscape);
  }, [wishlistOpen]);
  const selectVendor = (vendor: string) => { setSelectedVendor(vendor); setFilter("전체"); setVendorMenuOpen(false); const first = games.find((game) => vendor === "전체" || game.publisher === vendor); if (first) setSelectedGame(first); document.querySelector("#vendors")?.scrollIntoView({ behavior: "smooth", block: "start" }); };
  const addToWishlist = (game: Game) => {
    const id = gameId(game);
    if (wishlistIds.has(id)) { setWishlistOpen(true); return; }
    setWishlist((items) => [...items, { id, title: game.title, publisher: game.publisher, priority: items.length ? Math.max(...items.map((item) => item.priority)) + 1 : 1, addedAt: Date.now() }]);
  };
  const updateWishlistPriority = (id: string, priority: number) => setWishlist((items) => items.map((item) => item.id === id ? { ...item, priority: Math.max(1, priority || 1) } : item));
  const removeFromWishlist = (id: string) => setWishlist((items) => items.filter((item) => item.id !== id));

  return <main>
    <header className="nav"><a className="brand" href="#top">BGC<span>26</span></a><nav><a href="#map">MAP</a><a href="#events">주요 이벤트</a><div className={`vendor-menu ${vendorMenuOpen ? "open" : ""}`}><button className="vendor-menu-trigger" type="button" aria-expanded={vendorMenuOpen} aria-controls="vendor-menu-panel" onClick={() => setVendorMenuOpen((open) => !open)}>참가 업체 <span>↓</span></button><div className="vendor-menu-panel" id="vendor-menu-panel"><button className="vendor-menu-all" onClick={() => selectVendor("전체")}>전체 업체<small>{vendorTabs.length - 1} VENDORS</small></button>{vendorTabs.filter((vendor) => vendor !== "전체").map((vendor) => <button key={vendor} onClick={() => selectVendor(vendor)}>{vendor}<small>{games.filter((game) => game.publisher === vendor).length} GAMES</small></button>)}</div></div></nav><button className="wishlist-nav" type="button" onClick={() => setWishlistOpen(true)} aria-haspopup="dialog" aria-label={`구매희망 게임 ${wishlist.length}개 보기`}>구매희망 <b>{wishlist.length}</b></button><a className="nav-link" href="https://www.boardgamecon.com/" target="_blank" rel="noreferrer">OFFICIAL ↗</a></header>
    {wishlistOpen && <div className="wishlist-layer" role="presentation" onMouseDown={() => setWishlistOpen(false)}><aside className="wishlist-drawer" role="dialog" aria-modal="true" aria-label="구매희망 게임" onMouseDown={(event) => event.stopPropagation()}><header><div><span className="mono">LOCAL WISHLIST</span><h2>구매희망 게임 <em>{wishlist.length}</em></h2></div><button className="wishlist-close" type="button" onClick={() => setWishlistOpen(false)} aria-label="구매희망 목록 닫기">×</button></header><div className="wishlist-toolbar"><p>이 기기에만 저장됩니다.</p><label>정렬 <select value={wishlistSort} onChange={(event) => setWishlistSort(event.target.value as "priority" | "vendor")}><option value="priority">구매희망 순위</option><option value="vendor">업체명</option></select></label></div>{wishlist.length === 0 ? <div className="wishlist-empty"><strong>아직 담은 게임이 없습니다.</strong><p>게임 카드의 ‘구매희망에 담기’ 버튼으로 목록을 만들어 보세요.</p></div> : <div className="wishlist-groups">{Object.entries(wishlistGroups).map(([publisher, items]) => <section key={publisher}><h3>{publisher}<span>{items.length} GAMES</span></h3><ul>{items.map((item) => <li key={item.id}><div><b>{item.title}</b><span>구매희망 순위</span></div><label><input type="number" min="1" value={item.priority} onChange={(event) => updateWishlistPriority(item.id, Number(event.target.value))} aria-label={`${item.title} 구매희망 순위`} /></label><button type="button" onClick={() => removeFromWishlist(item.id)} aria-label={`${item.title} 목록에서 삭제`}>삭제</button></li>)}</ul></section>)}</div>}</aside></div>}
